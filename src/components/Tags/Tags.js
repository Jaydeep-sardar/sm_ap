...
const AddTagSection = styled(motion.div)`
  background: ${props => props.theme === 'golden' 
...
const Tags = () => {
  ...
  const handleAddTag = () => {
    if (newTag.trim()) {
      ...
      setTags([...tags, { id: newId, name: newTag.trim() }]);
      setNewTag('');
    }
  };
  ...
  const handleEditSave = (id) => {
    if (editValue.trim()) {
      setTags(tags.map(tag => 
        tag.id === id ? { ...tag, name: editValue.trim() } : tag
      ));
    }
    setEditingId(null);
    setEditValue('');
  ...
  return (
    ...
    >
      <Header>
        <Title theme={currentTheme}>Content Tag</Title>
      </Header>
      ...
      >
        <AddTagTitle theme={currentTheme}>Add Tag</AddTagTitle>
        <InputSection>
          <TagInput
            theme={currentTheme}
            type="text"
            ...
            {tags.length === 0 ? (
              ...
              tags.map((tag, index) => (
                ...
                >
                  ...
                  <EditableCell
                    theme={currentTheme}
                    onClick={() => editingId !== tag.id && handleEditStart(tag.id, tag.name)}
                  >
                    {editingId === tag.id ? (
                      <EditInput
                        theme={currentTheme}
                        ...
                        onKeyPress={(e) => handleEditKeyPress(e, tag.id)}
